...
class TH1InfoClass{
  public:
    ...
    void SetTitles(); 
    void Sumw2();
    TH1_type* GetTH1(std::string Name_);
...
void TH1InfoClass<TH1_type>::CreateTH1(edm::Service<TFileService> f){
  for(int i=0; i<TH1_Size_; i++){
    mapTH1[Var[i].Name] =f->make<TH1_type>(Var[i].Name.c_str(),"",Var[i].Bin, Var[i].Min, Var[i].Max);
  }
}

template<typename TH1_type> 
void TH1InfoClass<TH1_type>::CreateTH1( TFile* f, std::string dirName="" ){
  for(int i=0; i<TH1_Size_; i++){ 
    mapTH1[Var[i].Name] =(TH1_type*)f->Get( (dirName+Var[i].Name).c_str() );
  }
...
// Get Histogram
template<typename TH1_type> 
TH1_type* TH1InfoClass<TH1_type>::GetTH1(std::string Name_){
  return mapTH1.find(Name_)->second;
}

// Get Variables
template<typename TH1_type> 
TH1InfoVarClass TH1InfoClass<TH1_type>::GetVar(std::string Name_){
  return Var[indexTH1.find(Name_)->second];
}
template<typename TH1_type> 
TH1InfoVarClass TH1InfoClass<TH1_type>::GetVar(int index){
  return Var[index];
}
