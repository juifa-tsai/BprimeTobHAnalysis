...
double SFlight_CSVL_etamin[4] = {0.0, 0.5, 1.0, 1.5} ; 
double SFlight_CSVL_etamax[4] = {0.5, 1.0, 1.5, 2.4} ; 

double SFlight_CSVM_etamin[4] = {0.0, 0.8, 1.6} ; 
double SFlight_CSVM_etamax[4] = {0.8, 1.6, 2.4} ; 
...
TF1* plotmean(TString tagger, TString TaggerStrength, float Etamin, float Etamax, TString DataPeriod = "ABCD", TString opt = "" , int col = 1, float lineWidth = 1, int lineStyle = 1)
{
  ...
  //else std::cout << "NULL pointer returned... Function seems not to exist" << endl;
  return f;
}
...
TF1* GetSFLight(TString meanminmax, TString tagger, TString TaggerStrength, Float_t Etamin, Float_t Etamax, TString DataPeriod)
{
  ...
  //std::cout << sEtamin << endl;
  //std::cout << sEtamax << endl;

  if( (TaggerStrength == "L" || TaggerStrength == "M") && sEtamin == "0.0" && sEtamax == "2.4" )
  {
    std::cout << "For L and M taggers, the function is not provided integrated over eta. Only eta subranges are provided " << endl;
    return tmpSFl;
  }
  ...
  if( tmpSFl == NULL ) std::cout << "NULL pointer returned... Function seems not to exist" << endl;
