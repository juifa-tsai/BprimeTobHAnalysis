...
void _FinalShapePlots(){

	CMSstyle();

	//string Path="results/bin400GeV";
	string Path="results/bin300GeV";

	vector<string> hName, xTitle, yTitle, xUnit; vector<double> xmin, xmax;
	hName.push_back("HT"); xTitle.push_back("H_{T}"); yTitle.push_back("Events"); xUnit.push_back("GeV"); xmin.push_back(950); xmax.push_back(1850);
	...
	for( int h=0; h<hNameSize; h++){
		...
		string f = Path+"/Template_"+hName[h]+".root";
		...
		draw( f, c1, Path, b2,  hName[h], xTitle[h], yTitle[h], xUnit[h], xmin[h], xmax[h]-300, unblind );
		...
		draw( f, c1, Path, b2,  hName[h], xTitle[h], yTitle[h], xUnit[h], xmin[h], xmax[h]-300, blind );
	}
}
//CatAll_data_obs
void draw( string fs, TCanvas* c1, string save, int cate, string hName, string xTitle, string yTitle, string xUnit, double xmin, double xmax, bool isBlind ){
	...
	if( cate == b2 ) ax->SetNdivisions( 505 );; 
	...
	TLegend* leg = new TLegend(0.52, 0.7, 0.93, 0.92);
	leg->SetBorderSize(1);
	leg->SetTextFont(132);
	leg->SetLineColor(1);
	leg->SetLineStyle(1);
